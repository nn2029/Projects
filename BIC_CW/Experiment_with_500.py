...
# Population Size: 500, Iterations: 500
...
for i in range(10):
    f1 = cec2005.F1(10)
    pso = PSO(iters=500, pop_size=500, dimensions=10, informant_size=250, bounds=(-100, 100), func=f1)
    pso.initialize()
    optimizer = pso.optimize(alpha=0.9, beta=0.1, gamma=0.2, delta=0.7, verbose=False)
    f1_best_list_pso500.append(optimizer[2])
    optimizer7_pso = optimizer
f1_mean_pso500 = np.mean(f1_best_list_pso500)
print('Average Global Optima for F1 with PSO after 500 iterations with population 500 over 10 runs for PSO :', f1_mean_pso500, '\n')
...
for i in range(10):
    f1 = cec2005.F1(10)
    optimize = ga(func=f1, bounds=(-100, 100), num_dims=10, iters=500, pop_size=500, cross_rate = 90, mut_rate = 5, k=5, verbose=False)
    f1_best_list_ga500.append(optimize[2])
    optimizer7_ga = optimize
f1_mean_ga500 = np.mean(f1_best_list_ga500)
print('Average Global Optima for F1 with GA after a 500 iterations with population 500 over 10 runs :', f1_mean_ga500, '\n')
...
for i in range(10):
    f9 = cec2005.F9(10)
    pso = PSO(iters=500, pop_size=500, dimensions=10, informant_size=250, bounds=(-5, 5), func=f9)
    pso.initialize()
    optimizer = pso.optimize(alpha=0.9, beta=0.3, gamma=0.3, delta=0.4, verbose=False)
    f9_best_list_pso500.append(optimizer[2])
    optimizer8_pso = optimizer
f9_mean_pso500 = np.mean(f9_best_list_pso500)
print('Average Global Optima for F9 with PSO after 500 iterations with population 500 over 10 runs :', f9_mean_pso500, '\n')
...
for i in range(10):
    f9 = cec2005.F9(10)
    optimize = ga(func = f9, bounds = [-5,5], num_dims = 10, iters = 500, pop_size = 500, cross_rate = 90, mut_rate = 5 , k=5, verbose=False)
    f9_best_list_ga500.append(optimize[2])
    optimizer8_ga = optimize
f9_mean_ga500 = np.mean(f9_best_list_ga500)
print('Average Global Optima for F9 with GA after 500 iterations with population 500 over 10 runs :', f9_mean_ga500, '\n')

# F15 Benchmark Function
# PSO
f15_best_list_pso500 = []
optimizer9_pso = None
for i in range(5):
    f15 = cec2005.F15(10)
    pso = PSO(iters=1000, pop_size=500, dimensions=10, informant_size=250, bounds=(-5, 5), func=f15)
    pso.initialize()
    optimizer = pso.optimize(alpha=0.9, beta=0.3, gamma=0.8, delta=0.4, verbose=False)
    f15_best_list_pso500.append(optimizer[2])
    optimizer9_pso = optimizer
f15_mean_pso500 = np.mean(f15_best_list_pso500)
print('Average Global Optima for F15 with PSO after 500 iterations with population 500 over 5 runs :', f15_mean_pso500, '\n')
...
for i in range(5):
    f15 = cec2005.F15(10)
    optimize = ga(func = f15, bounds = [-5,5], num_dims = 10, iters = 500, pop_size = 500, cross_rate = 90, mut_rate = 5 , k=5, verbose=False)
    f15_best_list_ga500.append(optimize[2])
    optimizer9_ga = optimize
f15_mean_ga500 = np.mean(f15_best_list_ga500)
print('Average Global Optima for F15 with GA after 500 iterations with population 500 over 5 runs :', f15_mean_ga500, '\n')
...
# F1 function global optimum point over 500 iterations, 500 population
plt.plot(optimizer7_pso[0], optimizer7_pso[1], label="PSO")
plt.plot(optimizer7_ga[0], optimizer7_ga[3], label="GA")
...
# F9 function global optimum point over 500 iterations, 500 population
plt.plot(optimizer8_pso[0], optimizer8_pso[1], label = "PSO")
plt.plot(optimizer8_ga[0], optimizer8_ga[3], label="GA")
plt.grid(color='black', linestyle='--', linewidth=0.5)
plt.legend()
plt.ylabel('Global Optimum Best')
plt.xlabel('Iterations/ Generations')
...
# F15 function global optimum point over 500 iterations, 500 population
plt.plot(optimizer9_pso[0], optimizer9_pso[1], label = "PSO")
plt.plot(optimizer9_ga[0], optimizer9_ga[3], label="GA")
plt.grid(color='black', linestyle='--', linewidth=0.5)
plt.legend()
plt.ylabel('Global Optimum Best')
plt.xlabel('Iterations/ Generations')
