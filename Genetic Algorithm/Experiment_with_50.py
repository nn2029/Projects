...
# Population Size: 50, Iterations: 2000
...
for i in range(10):
    f1 = cec2005.F1(10)
    pso = PSO(iters=2000, pop_size=50, dimensions=10, informant_size=25, bounds=(-100, 100), func=f1)
    pso.initialize()
    optimizer = pso.optimize(alpha=0.9, beta=0.1, gamma=0.2, delta=0.7, verbose=False)
    f1_best_list_pso50.append(optimizer[2])
    optimizer4_pso = optimizer
f1_mean_pso50 = np.mean(f1_best_list_pso50)
print('Average Global Optima for F1 with PSO after 2000 iterations with population 50 over 10 runs for PSO :', f1_mean_pso50, '\n')
...
for i in range(10):
    f1 = cec2005.F1(10)
    optimize = ga(func=f1, bounds=(-100, 100), num_dims=10, iters=2000, pop_size=50, cross_rate = 90, mut_rate = 5, k=5, verbose=False)
    f1_best_list_ga50.append(optimize[2])
    optimizer4_ga = optimize
f1_mean_ga50 = np.mean(f1_best_list_ga50)
print('Average Global Optima for F1 with GA after 2000 iterations with population 50 over 10 runs :', f1_mean_ga50, '\n')
...
for i in range(10):
    f9 = cec2005.F9(10)
    pso = PSO(iters=2000, pop_size=50, dimensions=10, informant_size=25, bounds=(-5, 5), func=f9)
    pso.initialize()
    optimizer = pso.optimize(alpha=0.9, beta=0.3, gamma=0.3, delta=0.4, verbose=False)
    f9_best_list_pso50.append(optimizer[2])
    optimizer5_pso = optimizer
f9_mean_pso50 = np.mean(f9_best_list_pso50)
print('Average Global Optima for F9 with PSO after 2000 iterations with population 50 over 10 runs :', f9_mean_pso50, '\n')
...
for i in range(10):
    f9 = cec2005.F9(10)
    optimize = ga(func = f9, bounds = [-5,5], num_dims = 10, iters = 2000, pop_size = 50, cross_rate = 90, mut_rate = 5 , k=5, verbose=False)
    f9_best_list_ga50.append(optimize[2])
    optimizer5_ga = optimize
f9_mean_ga50 = np.mean(f9_best_list_ga50)
print('Average Global Optima for F9 with GA after 2000 iterations with population 50 over 10 runs :', f9_mean_ga50, '\n')
...
for i in range(5):
    f15 = cec2005.F15(10)
    pso = PSO(iters=2000, pop_size=50, dimensions=10, informant_size=25, bounds=(-5, 5), func=f15)
    pso.initialize()
    optimizer = pso.optimize(alpha=0.9, beta=0.3, gamma=0.8, delta=0.4, verbose=False)
    f15_best_list_pso50.append(optimizer[2])
    optimizer6_pso = optimizer
f15_mean_pso50 = np.mean(f15_best_list_pso50)
print('Average Global Optima for F15 with PSO after 2000 iterations with population 50 over 5 runs :', f15_mean_pso50, '\n')
...
for i in range(5):
    f15 = cec2005.F15(10)
    optimize = ga(func = f15, bounds = [-5,5], num_dims = 10, iters = 2000, pop_size = 50, cross_rate = 90, mut_rate = 5 , k=5, verbose=False)
    f15_best_list_ga50.append(optimize[2])
    optimizer6_ga = optimize
f15_mean_ga50 = np.mean(f15_best_list_ga50)
print('Average Global Optima for F15 with GA after 2000 iterations with population 50 over 5 runs :', f15_mean_ga50, '\n')

# F1 function global optimum point over 2000 iterations, 50 population
plt.plot(optimizer4_pso[0], optimizer4_pso[1], label="PSO")
plt.plot(optimizer4_ga[0], optimizer4_ga[3], label="GA")
...
# F9 function global optimum point over 2000 iterations, 50 population
plt.plot(optimizer5_pso[0], optimizer5_pso[1], label = "PSO")
plt.plot(optimizer5_ga[0], optimizer5_ga[3], label="GA")
plt.grid(color='black', linestyle='--', linewidth=0.5)
plt.legend()
plt.ylabel('Global Optimum Best')
plt.xlabel('Iterations/ Generations')
...
# F15 function global optimum point over 2000 iterations, 50 population
plt.plot(optimizer6_pso[0], optimizer6_pso[1], label = "PSO")
plt.plot(optimizer6_ga[0], optimizer6_ga[3], label="GA")
plt.grid(color='black', linestyle='--', linewidth=0.5)
plt.legend()
plt.ylabel('Global Optimum Best')
plt.xlabel('Iterations/ Generations')
